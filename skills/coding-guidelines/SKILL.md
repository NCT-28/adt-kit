...
description: Use before implementing or editing code — enforces think-before-coding, simplicity-first, surgical changes, goal-driven verification, and no-hardcoded-secrets. Loads the project's behavioral guidelines on demand.
---
...
After any bug fix or logic change → invoke the `verification` skill (calls the `verify` MCP tool) before claiming done.
